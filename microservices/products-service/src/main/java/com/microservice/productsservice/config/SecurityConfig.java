package com.microservice.productsservice.config;
...
@EnableMethodSecurity  // essa anotação ativa a segurança a nível de método em um aplicativo para que possa proteger métodos específicos com regras de segurança personalizadas. Isso permite controlar o acesso aos métodos com base em regras de autorização, como permissões de usuário, funções ou outras condições. Ao utilizar essa anotação pode configurar anotações como @PreAuthorize, @PostAuthorize, @Secured e outras para definir quem pode acessar determinados métodos com base em expressões de segurança.
